...
Ordinary code and configuration pull requests use the normal `pull_request` CI trigger. Generated pull requests contain only `data/processed/**`, so that duplicate trigger is ignored. The refresh workflow instead dispatches CI explicitly on the generated commit, waits for it to succeed, and records the successful run as the required `test` status on that exact commit. This avoids GitHub's `action_required` recursion guard while preserving the protected merge requirement and a direct link to the validating run.
